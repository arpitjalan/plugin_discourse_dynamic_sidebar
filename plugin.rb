    ...
    def solved_answers
      Rails.cache.fetch("latest_solved_answers", expires_in: 1.minute) do
        ...
        custom_field.each_with_index do |field, index|
          ...
          solved_answers[index] = {username: field.username, user_avatar: user.avatar_template_url.gsub("{size}", "25"), topic_title: field.topic.title, topic_url: field.topic.url, category_name: field.topic.category.name, post_created_at: field.post_created_at, solution_marked_at: field.created_at}
        end
        solved_answers
      end
    end
